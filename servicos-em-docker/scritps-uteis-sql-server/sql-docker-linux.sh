...
restaurarBackupDoBancoDeDados(){
	...
	docker exec docker-sqlserver /opt/mssql-tools/bin/sqlcmd -U sa -P 'Sa123456' -Q "Alter login [sa] with password=N'sa123456', CHECK_POLICY=OFF"
	echo "-> Verificando se existe e removendo base de dados antiga do container..."
	docker exec docker-sqlserver /opt/mssql-tools/bin/sqlcmd -U sa -P 'sa123456' -Q "IF EXISTS(SELECT * FROM sys.databases WHERE name= '${NOME_DA_BASE_DE_DADOS}') DROP DATABASE ${NOME_DA_BASE_DE_DADOS}"
	echo "-> Restaurando backup da nova base de dados no container..."
	docker exec docker-sqlserver /opt/mssql-tools/bin/sqlcmd -U sa -P 'sa123456' -Q "RESTORE DATABASE [${NOME_DA_BASE_DE_DADOS}] FROM DISK='/tmp/${NOME_DO_ARQUIVO_DE_BACKUP}' WITH MOVE '${NOME_DA_BASE_DE_DADOS}' TO '/var/opt/mssql/data/${NOME_DA_BASE_DE_DADOS}.mdf', MOVE '${NOME_DA_BASE_DE_DADOS}_log' TO '/var/opt/mssql/data/${NOME_DA_BASE_DE_DADOS}_log.ldf' "
...
copiarBackupDoContainerParaSuaMaquina(){
	...
	docker exec docker-sqlserver /opt/mssql-tools/bin/sqlcmd -U sa -P 'sa123456' -Q "BACKUP DATABASE ${NOME_DA_BASE_DE_DADOS} TO DISK='/var/opt/mssql/data/${NOME_DA_BASE_DE_DADOS}_$(date +%Y-%m-%d_%H-%M-%S).bak'"
